...
def test_create_relayed_v3_tx_invalid_data_value_for_sc_delegation(blockchain):
    ...
    5. Create and sign a relayed v3 transaction that includes the inner delegation contract transaction with invalid data.
    6. Attempt to send the relayed v3 transaction and verify that it fails due to invalid data.
    ...
    relayed_tx_data = "XXXX@XXX"  # Invalid data for the transaction

    # Create and sign relayed v3 transaction with invalid data
    relayed_v3_delegation_tx = create_and_sign_relayed_v3_transaction(
        ...
        data=relayed_tx_data,
    )
    try:
        delegation_tx_hash = send_transaction_and_check_for_fail(
            relayed_v3_delegation_tx
        )
    except GenericError as e:
        assert "transaction generation failed: invalid data field" in str(
            e
        ), f"Expected invalid data field error, got: {e}"
