...
def test_insufficient_gas_for_relayer(blockchain):
    """
    Test Steps:
    ...
    5. Create and sign a relayed v3 transaction that includes the inner delegation contract transaction with insufficient gas.
    6. Attempt to send the relayed v3 transaction and verify that it fails due to insufficient gas.
    ...
    # Create and sign relayed v3 transaction with insufficient gas
    ...
    try:
        ...
        assert "insufficient gas limit in tx" in str(
            e
        ), f"Expected gas limit error, got: {e}"
