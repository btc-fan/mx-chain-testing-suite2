...
def test_relayed_v3_create_new_delegation_sc_different_shards_negative(blockchain):
    """
    Test Steps:
    ...
    6. Send the relayed v3 transaction and verify it fails as expected due to different shards.
    7. Retrieve and verify the final nonces and balances for sender and relayer wallets.
       - Ensure the sender's nonce remains the same.
       - Ensure the relayer's nonce is incremented by 1.
       - Ensure the sender's balance remains the same.
    ...
    relayer_wallet = Wallet(Path(WALLETS_FOLDER + "/sd_1_wallet_key_6.pem"))

    # Set balances for wallets
    SENDER_AMOUNT = "1250" + "000000000000000000"  # 1250 EGLD in wei
    ...
    try:
        ...
        assert "transaction generation failed: sender shard mismatch" in str(
            e
        ), f"Expected sender shard mismatch error, got: {e}"
