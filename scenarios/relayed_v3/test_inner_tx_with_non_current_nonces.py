...
def test_inner_tx_with_non_current_nonces(blockchain):
    """
    Scenario:
    Send 5 inner transactions from the same sender:
    - None of them should have the current nonce (only lower and higher nonces).
    ...
    4. Create and sign 5 native EGLD transfer transactions (inner transactions) from sender to receiver with nonces that are not the current nonce.
    5. Create and sign a relayed v3 transaction that includes all 5 inner transactions.
    6. Send the relayed v3 transaction and verify its execution fails.
    7. Retrieve and verify the final nonces and balances for sender, receiver, and relayer wallets.
       - Ensure the sender's nonce is not incremented by the failed transactions.
       - Ensure the relayer's nonce is incremented by 1.
       - Ensure the sender's balance is not decremented by the failed transactions.
       - Ensure the receiver's balance is not incremented by the failed transactions.
    """
    ...
    assert sender_nonce_after == sender_nonce_before - 3
