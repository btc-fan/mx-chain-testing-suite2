...
def test_multiple_inner_tx_different_gas_price_negative(blockchain):
    """
    Test Steps:
    ...
    for amount in amounts:
        transfer_tx = create_and_sign_inner_transfer_tx(
            ...
            gas_price=int(random.randint(1000000000, 9000000000)),
        )
    ...
    try:
        ...
        assert "transaction generation failed: relayed tx v3 gas price mismatch" in str(
            e
    ...
    assert sender_nonce_before == sender_nonce_after
    ...
    assert relayer_nonce_before == relayer_nonce_after
