...
from core.get_validator_info import (
    get_keys_from_validator_auction,
    get_keys_from_validator_statistics,
)
from core.staking import stake, unStake
from models.key_management import add_blocks_until_key_eligible, add_key
...
def XXXtest_PR_6114_key_auction_process_through_epochs(blockchain):
    # === PRE-CONDITIONS ==============================================================
    ...
    # 4.4) we should have 39 keys with status "eligible"
    eligible_keys_in_epoch_x = get_keys_from_validator_statistics(
        needed_state="eligible"
    )
    ...
    # 5.2) 8 keys were selected from auction in epoch x and are now waiting
    all_auction_keys_from_epoch_x = (
        qualified_keys_in_epoch_x + not_qualified_keys_in_epoch_x
    )
