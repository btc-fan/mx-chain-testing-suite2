...
from core.delegation import (
    ...
    merge_validator_to_delegation_same_owner,
    stake_nodes,
)
from core.get_delegation_info import get_delegation_contract_address_from_tx
from core.staking import stake
from models.key_management import add_key
...
# TODO The test below is not executed as it fails and restricts user to merge their code


@pytest.mark.parametrize("epoch", EPOCHS_ID, indirect=True, ids=epoch_id)
def XXXtest_70_75_validator_to_sp_transitions_and_validations(blockchain, epoch):
    # === PRE-CONDITIONS ==============================================================
    ...
    key_1 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_1.pem"))
    key_2 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_2.pem"))
    A_keys = [key_1, key_2]

    _B = Wallet(Path(WALLETS_FOLDER + "/sd_2_wallet_key_2.pem"))
    key_3 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_3.pem"))
    key_4 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_4.pem"))
    B_keys = [key_3, key_4]

    _C = Wallet(Path(WALLETS_FOLDER + "/sd_1_wallet_key_3.pem"))
    key_5 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_5.pem"))
    key_6 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_6.pem"))
    C_keys = [key_5, key_6]

    _D = Wallet(Path(WALLETS_FOLDER + "/sd_2_wallet_key_4.pem"))
    key_7 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_7.pem"))
    key_8 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_8.pem"))
    D_keys = [key_7, key_8]

    _E = Wallet(Path(WALLETS_FOLDER + "/sd_0_wallet_key_5.pem"))
    key_9 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_9.pem"))
    key_10 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_10.pem"))
    E_keys = [key_9, key_10]

    _F = Wallet(Path(WALLETS_FOLDER + "/sd_1_wallet_key_6.pem"))
    key_11 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_11.pem"))
    key_12 = ValidatorKey(Path(VALIDATOR_KEYS_FOLDER + "/validatorKey_12.pem"))
    F_keys = [key_11, key_12]

    _G = Wallet(Path(WALLETS_FOLDER + "/sd_1_wallet_key_7.pem"))
